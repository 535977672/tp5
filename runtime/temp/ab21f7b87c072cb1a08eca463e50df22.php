<?php /*a:2:{s:52:"D:\mf\pcnfc\application\admin\view\goods\_brand.html";i:1543196952;s:53:"D:\mf\pcnfc\application\admin\view\public\layout.html";i:1542359974;}*/ ?>
<!doctype html>
<html>
<head>
  ...
  <div class="fixed-bar">
    ...
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
          <input type="text" placeholder="名称" class="input-txt" id="brand_name" name="name" value="<?php echo htmlentities($brand['name']); ?>">
	  ...
	  <dl class="row">
        ...
        <dd class="opt">
          <input type="text" class="input-txt" name="url" value="<?php echo htmlentities($brand['url']); ?>">
          ...
          <div id="gcategory">
           <select name="parent_cat_id" id="parent_id_1" onblur="get_category(this.value,'parent_id_2','0');"  onchange="get_category(this.value,'parent_id_2','0');" class="form-control" >
                ...
                <?php if(is_array($cat_list) || $cat_list instanceof \think\Collection || $cat_list instanceof \think\Paginator): if( count($cat_list)==0 ) : echo "" ;else: foreach($cat_list as $key=>$v): ?>
                    <option value="<?php echo htmlentities($v['id']); ?>"  <?php if($v['id'] == $brand['parent_cat_id']): ?> selected="selected" <?php endif; ?>><?php echo htmlentities($v['name']); ?></option>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="input-file-show">
              <span class="show">
                  <a id="img_a" target="_blank" class="nyroModal" rel="gal" href="<?php echo htmlentities($brand['logo']); ?>">
                    <i id="img_i" class="fa fa-picture-o" onmouseover="layer.tips('<img src=<?php echo htmlentities($brand['logo']); ?>>',this,{tips: [1, '#fff']});" onmouseout="layer.closeAll();"></i>
                  </a>
              </span>
              <span class="type-file-box">
                  <input type="text" id="logo" name="logo" value="<?php echo htmlentities($brand['logo']); ?>" class="type-file-text">
                  <input type="button" name="button" id="button1" value="选择上传..." class="type-file-button">
                  <input class="type-file-file" onClick="GetUploadify(1,'brand_logo','brand','img_call_back')" size="30" hidefocus="true" nc_type="change_site_logo"
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <input type="text" placeholder="排序" name="sort" value="<?php echo htmlentities($brand['sort']); ?>" maxlength="3" class="input-txt">
	  ...
	  <dl class="row">
        ...
        <dd class="opt">
          ...
          <textarea rows="4" cols="60" name="desc" class="input-txt"><?php echo htmlentities($brand['desc']); ?></textarea>
      ...
      <div class="bot"><a href="JavaScript:void(0);" onClick="verifyForm();" class="ncap-btn-big ncap-btn-green" id="submitBtn">确认提交</a></div>
        ...
        <input type="hidden" name="id" value="<?php echo htmlentities($brand['id']); ?>">
        <input type="hidden" name="p" value="<?php echo htmlentities(app('request')->get('p')); ?>">
  </form>
</div>
<script>
function verifyForm(){
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo url('Admin/Goods/addEditBrand'); ?>",
        ...
        success: function (data) {
            if (data.status == 1) {
                layer.msg(data.msg, {icon: 1},function () {
                    location.href = "<?php echo url('Admin/Goods/brandList'); ?>";
                });
            } else {
...
window.onload = function(){
    var brand_cat_id = parseInt('<?php echo htmlentities($brand['cat_id']); ?>');
	if(brand_cat_id > 0 ){
